...
exports.postCommand = async (req, res) => {
    try{
        ...
        const result = await arduinoService.postCommand(req, res);
        restService.successHandler(res, 'Post command success', result)
    } catch(error){
        restService.errorHandler(res, 'Post command failed', error)
    }
}
